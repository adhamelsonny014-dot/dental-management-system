...
const RoleBadge = ({ role }) => {
  ...
  return (
    <span
      className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${map[role] || "bg-slate-100 text-slate-600"}`}
    >
      {role}
    </span>
  );
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-lg bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {/* Basic */}
          <div>
            <p className="text-xs font-semibold text-dental-muted uppercase tracking-wide mb-3">
              Basic information
            </p>
                ...
                <div key={slot.day} className="flex items-center gap-3">
                  <label className="flex items-center gap-2 cursor-pointer w-32">
                    <div className="relative flex-shrink-0">
                      ...
                      />
                      <div
                        className={`w-9 h-5 rounded-full transition-colors ${slot.open ? "bg-primary-600" : "bg-slate-200"}`}
                      />
                      <div
                        className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${slot.open ? "translate-x-4" : ""}`}
                      />
                    </div>
                    <span
                      className={`text-sm capitalize ${slot.open ? "text-slate-700 font-medium" : "text-dental-muted"}`}
                    >
                      {slot.day}
                    </span>
                  </label>
                  {slot.open ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        value={slot.start}
                        onChange={(e) => setSchedule(i, "start", e.target.value)}
                      />
                      ...
                      <input
                        ...
                        className="input w-24 text-sm py-1"
                        value={slot.end}
                        onChange={(e) => setSchedule(i, "end", e.target.value)}
                      />
                    </div>
                  ) : (
...
const StaffManagement = () => {
  ...
  const handleCreateLogin = async (member) => {
    ...
    try {
      ...
      const { email, tempPassword } = res.data.login;
      if (tempPassword) {
        // Shown once — the admin shares it with the staff member
        toast.success(
          <span>
            Login created for <b>{email}</b>
            <br />
        ...
        toast.success(`Linked to the existing login for ${email}`);
      }
    } catch (err) {
          ...
          {staff.map((m) => (
            <div key={m._id} className="card p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                {m.specialization && (
                  <p className="text-xs text-dental-muted mt-1 truncate">{m.specialization}</p>
                )}
                {m.phone && <p className="text-xs text-dental-muted mt-0.5">{m.phone}</p>}
              </div>

              <div className="flex flex-col gap-1">
                <button onClick={() => setDrawerMember(m)} className="btn-ghost text-xs px-2 py-1">
                  Edit
                </button>
                {!m.userId && ["dentist", "assistant", "hygienist", "receptionist"].includes(m.role) && (
                  <button
                    type="button"
