...
import { currencyRounded, percent as formatPercent } from "../utils/format";
      ...
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2">
          ...
          <span className="font-medium text-slate-800">
            {prefix === "$" ? currencyRounded(p.value) : p.value}
          </span>
        </div>
      ))}
    </div>
...
const Reports = () => {
  ...
  return (
    <div className="p-8">
      <PageHeader title="Reports & Analytics" subtitle="Data aggregated from all clinic modules" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <KPICard
          ...
          sub={`+${overview?.newPatients ?? 0} new this period`}
          color="text-blue-600"
          icon="👥"
        />
        <KPICard
          ...
          sub={`${overview?.completedAppointments ?? 0} completed`}
          color="text-emerald-600"
          icon="📅"
        />
        <KPICard
          label="Revenue collected"
          value={overview ? currencyRounded(overview.totalCollected) : "—"}
        ...
        <KPICard
          label="No-show rate"
          value={overview ? formatPercent(overview.noShowRate) : "—"}
          sub={`${overview?.noShowAppointments ?? 0} no-shows`}
          color={overview?.noShowRate > 10 ? "text-red-600" : "text-slate-700"}
          icon="🚫"
        />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <KPICard
          ...
          value={overview ? currencyRounded(overview.totalInvoiced) : "—"}
          sub={`${overview?.totalInvoices ?? 0} invoices`}
        ...
        <KPICard
          ...
          value={planSummary?.totalPlans ?? "—"}
          sub={`${planSummary?.totalCharts ?? 0} dental charts`}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 mb-5">
        ...
        <ChartCard
          ...
          subtitle="Completed, cancelled, and no-shows"
          loading={loading.apptMonth}
        >
          <ResponsiveContainer width="100%" height={260}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 mb-5">
        <ChartCard
          ...
          subtitle="Distribution of visit categories"
          loading={loading.apptType}
        >
          {apptTypeData.length === 0 ? (
            <div className="flex items-center justify-center h-48 text-dental-muted text-sm">
        ...
        <ChartCard
          ...
          subtitle="New registrations and cumulative total"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 mb-5">
        <ChartCard
          title="Revenue by dentist"
          subtitle="Total invoiced per practitioner"
          loading={loading.dentist}
        >
                ...
                return (
                  <div key={m.name}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-sm font-bold text-slate-800">{currencyRounded(m.total)}</span>
                    </div>
      ...
      <ChartCard
        title="Top procedures by revenue"
        subtitle="Most revenue-generating line items on invoices"
        loading={loading.procedures}
      >
        {procedureData.length === 0 ? (
          <div className="flex items-center justify-center h-32 text-dental-muted text-sm">
            No invoice data yet
          </div>
        ) : (
                  ...
                  return (
                    <div className="bg-white border border-dental-border rounded-xl shadow-lg p-3 text-sm max-w-xs">
                      ...
                      <p className="text-dental-muted">
                        Revenue:{" "}
                        <span className="font-bold text-slate-800">{currencyRounded(payload[0]?.value)}</span>
                      </p>
                      <p className="text-dental-muted">
                        Count: <span className="font-bold text-slate-800">{payload[1]?.value}</span>
                      </p>
                    </div>
                  );
                }}
