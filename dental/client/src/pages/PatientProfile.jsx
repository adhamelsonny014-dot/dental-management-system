...
const InfoRow = ({ label, value }) => (
  <div className="flex flex-col sm:flex-row sm:items-center py-2.5 border-b border-dental-border last:border-0">
    <span className="text-xs font-semibold text-dental-muted uppercase tracking-wide w-40 flex-shrink-0 mb-0.5 sm:mb-0">
      {label}
    </span>
...
const PatientProfile = () => {
  ...
  if (!patient) return <div className="p-8 text-center text-dental-muted">Patient not found.</div>;
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2 flex-wrap">
            <button className="btn-ghost text-sm" onClick={() => navigate("/patients")}>
              ← Back
            </button>
            <button
            ...
            <button
              className="btn-ghost text-sm border border-dental-border"
              onClick={() => navigate(`/clinical-notes/${id}`)}
            >
              Clinical notes
            </button>
            <button
            ...
            <button
              className="btn-ghost text-sm border border-dental-border"
              onClick={() => navigate(`/invoices?patient=${id}`)}
            >
              Invoices
            </button>
            <button className="btn-primary text-sm" onClick={() => navigate(`/patients/${id}/edit`)}>
              Edit patient
            </button>
          </div>
        }
      ...
      {tab === "Overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="card p-5">
            <h3 className="text-xs font-semibold text-dental-muted uppercase tracking-wide mb-3">
              Contact information
            </h3>
          ...
          <div className="card p-5">
            ...
            {patient.conditions?.length > 0 ? (
              patient.conditions.map((c) => (
                <div
                  key={c}
                  className="flex items-center gap-2 py-1.5 border-b border-dental-border last:border-0"
                >
      ...
      {tab === "Documents" && (
        <div className="card p-12 flex flex-col items-center text-center">
          ...
          <p className="text-dental-muted text-sm mt-1">
            X-rays and imaging uploads will appear here (Week 3)
          </p>
        </div>
      )}
    </div>
