  ...
  const kpis = isDentist
    ? [
        ...
        },
        {
          ...
          sub: `${stats?.totalCharts ?? 0} dental charts`,
    ...
    : [
        ...
        {
          label: "Pending web bookings",
          value: stats?.pendingBookings ?? "—",
          sub: `${stats?.activeTreatmentPlans ?? 0} active treatment plans`,
          icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z",
          color: "bg-amber-500",
        },
      ];
    ...
    <section className="p-8">
      <header className="mb-8">
        ...
        <p className="text-dental-muted text-sm mt-1">
          {isDentist
            ? "Your patients, treatment plans, and schedule at a glance."
            : "Clinic overview — patients, charts, bookings, and appointments."}
        </p>
      </header>
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {todayAppts.map((a) => (
              ...
              >
                ...
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize flex-shrink-0 ${STATUS_COLOR[a.status] || "bg-slate-100 text-slate-600"}`}
                >
                  {a.status?.replace(/-/g, " ")}
                </span>
              </article>
