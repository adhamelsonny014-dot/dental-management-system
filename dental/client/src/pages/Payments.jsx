  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        {Object.entries(byMethod).slice(0, 3).map(([method, amt]) => (
          <div key={method} className="card p-4">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-lg">{METHOD_ICONS[method]}</span>
              <span className={`text-xs font-medium px-1.5 py-0.5 rounded capitalize ${METHOD_STYLES[method]}`}>
                {method.replace("-"," ")}
              </span>
            </div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {METHODS.map((m) => (
          <button key={m}
            ...
            {m ? <>{METHOD_ICONS[m]} {m.replace("-"," ")}</> : "All methods"}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-12 flex flex-col items-center text-center">
          <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center mb-3 text-2xl">💳</div>
              ...
              <tbody>
                {payments.map((p, i) => (
                  <tr key={p._id}
                    ...
                    <td className="px-4 py-3">
                      <span className={`flex items-center gap-1.5 text-xs font-medium px-2 py-0.5 rounded-full capitalize w-fit ${METHOD_STYLES[p.method]}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.45)" }}>
          <div className="card w-full max-w-sm p-6">
            ...
            <p className="text-sm text-dental-muted mb-1">
              This will reverse <span className="font-semibold text-red-600">{currency(deleteTarget.amount)}</span> from invoice{" "}
            ...
            <div className="flex gap-3 justify-end">
              <button className="btn-ghost" onClick={() => { setDeleteTarget(null); setConfirmText(""); }}>
                Cancel
              </button>
              <button
