...
const QueueCard = ({ appointment, onStatusChange, updating }) => {
  ...
  return (
    <div className={`card p-5 transition-all ${appointment.status === "in-progress" ? "ring-2 ring-amber-400" : ""}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-dental-muted">{p?.patientNumber} · {p?.phone || "No phone"}</p>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <StatusBadge status={appointment.status} />
              <span className="text-xs text-dental-muted capitalize">{appointment.type?.replace(/-/g," ")}</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-dental-border">
        <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: dr?.color || "#3b82f6" }} />
        <span className="text-xs text-dental-muted">Dr. {dr?.firstName} {dr?.lastName}</span>
        {appointment.reason && (
          <span className="text-xs text-dental-muted truncate ml-auto">"{appointment.reason}"</span>
        )}
      </div>
...
const WaitingRoom = () => {
  ...
  const sorted = [...filtered].sort((a, b) => {
    const priority = { "in-progress": 0, confirmed: 1, scheduled: 2, completed: 3, "no-show": 4, cancelled: 5 };
